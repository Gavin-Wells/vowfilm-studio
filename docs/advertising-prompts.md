...
原始 `SKILL.md`、`references/ecommerce.md`、`references/patterns.md` 和 agent 元数据保存在 `server/internal/advertising/prompts/`，文件哈希在 `source.json`。前三份文件完整嵌入导演系统指令，模板版本按三份内容共同计算：`ad-video-prompt-v3:a3bd10452fa1`。源码编译后不依赖本机 ZIP 路径、数据库类型或历史租户资料。

## 生成链路

1. 当前商品资料、自定义要求与实际图片顺序进入一次文本模型调用，生成一条包含00:00–00:15切镜、动作、台词、声音归属和一致性要求的完整中文 Prompt。JSON 外层仅服务工程保存与界面展示。
